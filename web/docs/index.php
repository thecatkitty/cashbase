<?php
error_reporting(error_reporting() - 8);
/********************************
...
<html>
 <head>
  ...
  <link rel="stylesheet" type="text/css" href="http://cdn.celones.pl/sskin/screen.php?c=||gray||green|"/>
<style>
...
$(function(){
	...
	function list() {
		...
		$.get('?',{'do':'list','file':hashval},function(data) {
			...
			if(data.success) {
				...
				!data.results.length && $tbody.append('<tr><td class="empty" colspan=5>Ten folder jest pusty</td></tr>')
	...
	function renderFileRow(data) {
		...
		var $dl_link = $('<a/>').attr('href','?do=download&file='+encodeURIComponent(data.path))
			.addClass('download').text('pobierz');
		var $delete_link = $('<a href="#" />').attr('data-file',data.path).addClass('delete').text('usuń');
		var perms = [];
		if(data.is_readable) perms.push('odczyt');
		if(data.is_writable) perms.push('zapis');
		if(data.is_executable) perms.push('wykonanie');
		var $html = $('<tr />')
	...
	function renderBreadcrumbs(path) {
		var base = "",
			$html = $('<div/>').append( $('<a href=#>Katalog główny</a></div>') );
	...
	function formatTimestamp(unix_timestamp) {
		var m = ['sty', 'lut', 'mar', 'kwi', 'maj', 'cze', 'lip', 'sie', 'wrz', 'paź', 'lis', 'gru'];
	...
	function formatFileSize(bytes) {
		var s = ['bajtów', 'KB','MB','GB','TB','PB','EB'];
		for(var pos = 0;bytes >= 1000; pos++,bytes /= 1024);
		var d = Math.round(bytes*10);
		return pos ? [parseInt(d/10),".",d%10," ",s[pos]].join('') : bytes + ' bajtów';
	}
})
 ...
 <body>
  <header>
   <h1>Baza przychodów i wydatków</h1>
   <i>Baza przychodów i wydatków z podziałem na kategorie oraz systemem dokumentów.</i>
  </header>
  <section>
   <article>
    <h2>Dokumenty</h2>
    <div id="top">
	 <form action="?" method="post" id="mkdir" />
      <label for="dirname">Utwórz nowy folder</label>
	  <input id="dirname" type="text" name="name" value="" />
      <input type="submit" value="utwórz" />
	 </form>
	 <div id="file_drop_target">
      Przeciągnij pliki tutaj, by dodać
      <b>albo</b>
   ...
   <article>
	<input type="button" onclick="location.href='../index.php'" value="Wróć do strony głównej" style="display: block;width: 50%; margin: 1em auto" />
   </article>
  </section>
  <footer>
   <p>simple php filemanager by <a href="https://github.com/jcampbell1">jcampbell1</a></p>
